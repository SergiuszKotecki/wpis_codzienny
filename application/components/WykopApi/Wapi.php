...
namespace app\components\WykopApi;
...
class Wapi
{
    ...
    public function doRequest($action, $postData = null, $filesData = null)
    {
        ...
        $sign = $this->sign($url, $postData);
        if ($filesData !== null) {
            $postData = $postData + $filesData;
        }
        $result = $this->curl($url, $postData, $sign);
    ...
    protected function curl($url, $post = null, $sign)
    {
        $options = array(
